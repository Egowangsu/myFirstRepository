...
<script type="text/javascript">
	...
	function getRemarkMss(){
		$.ajax({
			url:"workbench/activity/getRemarkMss.do" ,
			data:{
				id:"${activity.id}"
			} ,
	...
	}       //这个删除图标绑定事件的参数有点特殊,因为是动态生成的，所以要以字符串的形式添加参数
	      deleteRemark=function (id) {
            if(window.confirm("确定要删除该备注吗?")){
                $.ajax({
   ...
   saveBtn=function () {
       $.ajax({
           ...
           data:{
               ...
               activityId:"${activity.id}"                   <!--在获得detail时往request域中仍了当前的activity信息-->
           },
           type:"post",   //增删改、登陆操作用post，正常取值用get
           dataType:"json" ,
           success:function (data) {
    ...
    //为备注修改的更新按钮绑定事件
        $("#updateRemarkBtn").click(function () {
            $.ajax({
                ...
                data:{
                    id:id,
                    noteContent:$("#noteContent").val()  //这是当前备注的修改框内容
                } ,
                type:"post",   //增删改、登陆操作用post，正常取值用get
                dataType:"json" ,
                success:function (data) {
...
<%--			<div style="position: relative; top: -40px; left: 40px;" id="beizhu1" >--%>
<%--				<h5>哎呦！</h5>--%>
